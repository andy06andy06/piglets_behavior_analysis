...
class Dataset_CRNN(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, data_path, folders, labels, frames, action_names, transform=None):
        "Initialization"
        self.data_path = data_path
        self.labels = labels
        self.folders = folders
        self.transform = transform
        self.frames = frames
        self.action = action_names

    def __len__(self):
        "Denotes the total number of samples"
        return len(self.folders)

    def read_images(self, path, selected_folder, label, use_transform):
        X = []
        for i in self.frames:
            image = Image.open(os.path.join(path, self.action[label], 'image', selected_folder, '{:04d}.jpg'.format(i)))
    ...
    def __getitem__(self, index):
        ...
        # Load data
        X = self.read_images(self.data_path, folder, label, self.transform)     # (input) spatial images
        y = torch.LongTensor([self.labels[index]])                  # (labels) LongTensor are for int64 instead of FloatTensor
...
class ResCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, drop_p=0.3, CNN_embed_dim=300):
...
# 2D CNN encoder using MobileNetV2 pretrained
class MobileCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, drop_p=0.3, CNN_embed_dim=300):
        """Load the pretrained ResNet-152 and replace top fc layer."""
        super(MobileCNNEncoder, self).__init__()

        self.fc_hidden1 = fc_hidden1
        self.drop_p = drop_p

        self.mobilenetv2 = models.mobilenet_v2(pretrained=True)
        self.fc1 = nn.Linear(self.mobilenetv2.classifier[1].in_features, fc_hidden1)
        self.bn1 = nn.BatchNorm1d(fc_hidden1, momentum=0.01)
        self.fc2 = nn.Linear(fc_hidden1, CNN_embed_dim)
    ...
    def forward(self, x_3d):
        ...
        for t in range(x_3d.size(1)):
            # ResNet CNN
            with torch.no_grad():
                x = self.mobilenetv2.features(x_3d[:, t, :, :, :])  # ResNet
...
class EfficientNetCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, drop_p=0.3, CNN_embed_dim=300):
        """Load the pretrained ResNet-152 and replace top fc layer."""
        super(EfficientNetCNNEncoder, self).__init__()
        ...
        self.fc1 = nn.Linear(self.efficientnet._fc.in_features, fc_hidden1)
        self.bn1 = nn.BatchNorm1d(fc_hidden1, momentum=0.01)
        self.fc2 = nn.Linear(fc_hidden1, CNN_embed_dim)
    ...
    def forward(self, x_3d):
        ...
        for t in range(x_3d.size(1)):
            # ResNet CNN
            with torch.no_grad():
                x = self.efficientnet.extract_features(x_3d[:, t, :, :, :])  # ResNet
                x = nn.functional.adaptive_avg_pool2d(x, 1).reshape(x.shape[0], -1)
